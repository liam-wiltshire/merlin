...
<head>
    <title>{{ name }}</title>
<base href="http://www.pa-rainbows.com/arthur/" />
...
{% block headerbar %}{% include "headerbar.tpl" %}{% endblock%}
    ...
    <ul class="menulist" id="listMenuRoot">
        {% for drop in menu %}
        <li>
            <a href="/arthur/{{ drop.1 }}"{% if drop.2 %} target="_blank"{% endif %}>{{ drop.0 }}</a>
            <ul>
            {% if drop.3 %}
                {% for sub in drop.3 %}
                <li><a href="/arthur/{{ sub.1 }}"{% if sub.2 %} target="_blank"{% endif %}>{{ sub.0 }}</a></li>
                {% endfor%}
            {% endif %}
            </ul>
        </li>
